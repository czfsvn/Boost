...
struct async_write_message_initiation
{
    ...
    void operator()(CompletionHandler&& completion_handler, tcp::socket& socket, std::unique_ptr<std::string> encoded_message) const
    {
        ...
        boost::asio::async_write(socket, encoded_message_buffer, intermediate_completion_handler{ socket, std::move(encoded_message), std::forward<CompletionHandler>(completion_handler) });
    }
};

template <typename T, typename CompletionToken>
auto async_write_message(tcp::socket& socket, const T& message, CompletionToken&& token)
    ...
    -> typename boost::asio::async_result<typename std::decay<CompletionToken>::type, void(boost::system::error_code)>::return_type
{
    ...
    return boost::asio::async_initiate<CompletionToken, void(boost::system::error_code)>(async_write_message_initiation(), token, std::ref(socket), std::move(encoded_message));
}
...
void test_callback()
{
    ...
    async_write_message(socket, 123456, [](const boost::system::error_code& error) {
        if (!error)
        {
...
void test_future()
{
    ...
    std::future<void> f = async_write_message(socket, 654.321, boost::asio::use_future);
