...
auto async_write_message(tcp::socket& socket, const char* message, bool allow_partial_write, CompletionToken&& token)
    ...
    -> typename boost::asio::async_result<typename std::decay<CompletionToken>::type, void(boost::system::error_code, std::size_t)>::return_type
{
    ...
    if (allow_partial_write)
    {
        ...
        return socket.async_write_some(boost::asio::buffer(message, std::strlen(message)), std::forward<CompletionToken>(token));
    }
    else
    {
        // As above, we must perfectly forward the completion token when calling
        // the alternate underlying operation.
        return boost::asio::async_write(socket, boost::asio::buffer(message, std::strlen(message)), std::forward<CompletionToken>(token));
    }
}
...
void test_callback()
{
    ...
    async_write_message(socket, "Testing callback\r\n", false, [](const boost::system::error_code& error, std::size_t n) {
        if (!error)
        {
