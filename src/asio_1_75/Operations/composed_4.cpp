...
struct async_write_message_initiation
{
    ...
    void operator()(CompletionHandler&& completion_handler, tcp::socket& socket, const char* message) const
    {
        ...
        auto executor = boost::asio::get_associated_executor(completion_handler, socket.get_executor());
        ...
        if (length == 0)
        {
            boost::asio::post(boost::asio::bind_executor(executor, std::bind(std::forward<CompletionHandler>(completion_handler), boost::asio::error::invalid_argument)));
        }
        else
        {
            boost::asio::async_write(socket, boost::asio::buffer(message, length),
                                     boost::asio::bind_executor(executor, std::bind(std::forward<CompletionHandler>(completion_handler), std::placeholders::_1)));
        }
    }
};
    ...
    -> typename boost::asio::async_result<typename std::decay<CompletionToken>::type, void(boost::system::error_code)>::return_type
{
    ...
    return boost::asio::async_initiate<CompletionToken, void(boost::system::error_code)>(async_write_message_initiation(), token, std::ref(socket), message);
}
...
void test_callback()
{
    ...
    async_write_message(socket, "", [](const boost::system::error_code& error) {
        if (!error)
        {
...
void test_future()
{
    ...
    std::future<void> f = async_write_message(socket, "", boost::asio::use_future);
