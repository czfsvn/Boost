...
struct close_after
{
    close_after(std::chrono::steady_clock::duration t, tcp_socket& s) : timeout_(t), socket_(s) {}
...
namespace boost
{
    namespace asio
    {
        ...
        public:
            ...
            {
            public:
                completion_handler_type(const close_after& token) : token_(token) {}
            ...
            explicit async_result(completion_handler_type& h) : timeout_(h.token_.timeout_), socket_(h.token_.socket_)
            {
                h.error_ = &error_;
                h.t_     = &t_;
            }
            ...
            return_type get()
            {
                boost::asio::io_context& io_context = boost::asio::query(socket_.get_executor(), boost::asio::execution::context);
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        boost::asio::async_connect(socket, endpoints, close_after(std::chrono::seconds(10), socket));
        ...
        boost::asio::async_write(socket, boost::asio::buffer(msg), close_after(std::chrono::seconds(10), socket));

        for (std::string input_buffer;;)
        {
            // Run an asynchronous read operation with a timeout.
            std::size_t n = boost::asio::async_read_until(socket, boost::asio::dynamic_buffer(input_buffer), '\n', close_after(std::chrono::seconds(10), socket));
