...
{
public:
    session(stream_protocol::socket sock) : socket_(std::move(sock)) {}
...
{
public:
    server(boost::asio::io_context& io_context, const std::string& file) : acceptor_(io_context, stream_protocol::endpoint(file))
    {
        do_accept();
    }
