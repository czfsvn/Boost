...
{
public:
    tcp_session(tcp::socket socket, channel& ch) : channel_(ch), socket_(std::move(socket))
    {
...
private:
    ...
    {
        auto self(shared_from_this());
        non_empty_output_queue_.async_wait([this, self](const boost::system::error_code& /*error*/) {
            // Check if the session was stopped while the operation was pending.
            if (stopped())
                return;
    ...
    void write_line()
    {
        ...
        auto self(shared_from_this());
        boost::asio::async_write(socket_, boost::asio::buffer(output_queue_.front()), [this, self](const boost::system::error_code& error, std::size_t /*n*/) {
            // Check if the session was stopped while the operation was pending.
            if (stopped())
                return;
...
{
public:
    udp_broadcaster(boost::asio::io_context& io_context, const udp::endpoint& broadcast_endpoint) : socket_(io_context)
    {
...
{
public:
    server(boost::asio::io_context& io_context, const tcp::endpoint& listen_endpoint, const udp::endpoint& broadcast_endpoint) : io_context_(io_context), acceptor_(io_context, listen_endpoint)
    {
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        udp::endpoint broadcast_endpoint(boost::asio::ip::make_address(argv[2]), atoi(argv[3]));
