...
private:
    void do_operations()
    {
        ...
        if (session_impl_.want_read() && !read_in_progress_)
        {
            ...
            socket_.async_wait(tcp::socket::wait_read, [this, self](boost::system::error_code ec) {
        ...
        if (session_impl_.want_write() && !write_in_progress_)
        {
            ...
            socket_.async_wait(tcp::socket::wait_write, [this, self](boost::system::error_code ec) {
...
{
public:
    server(boost::asio::io_context& io_context, unsigned short port) : acceptor_(io_context, { tcp::v4(), port })
    {
        do_accept();
    }
