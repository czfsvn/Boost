...
struct async_write_messages_implementation
{
    ...
    void operator()(Self& self, const boost::system::error_code& error = boost::system::error_code(), std::size_t = 0)
    {
        if (!error)
        {
            switch (state_)
            ...
            case waiting:
                state_ = writing;
                boost::asio::async_write(socket_, boost::asio::buffer(*encoded_message_), std::move(self));
                return;  // Composed operation not yet complete.
            }
        }
    ...
    -> typename boost::asio::async_result<typename std::decay<CompletionToken>::type, void(boost::system::error_code)>::return_type
{
    ...
    return boost::asio::async_compose<CompletionToken, void(boost::system::error_code)>(
        async_write_messages_implementation{ socket, std::move(encoded_message), repeat_count, std::move(delay_timer), async_write_messages_implementation::starting }, token, socket);
}
