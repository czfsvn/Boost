...
#define BOOST_ASIO_ALSO_INHERIT_TRACKED_HANDLER , public ::custom_tracking::tracked_handler

#define BOOST_ASIO_HANDLER_TRACKING_INIT ::custom_tracking::init()

#define BOOST_ASIO_HANDLER_LOCATION(args) ::custom_tracking::location args

#define BOOST_ASIO_HANDLER_CREATION(args) ::custom_tracking::creation args

#define BOOST_ASIO_HANDLER_COMPLETION(args) ::custom_tracking::completion tracked_completion args

#define BOOST_ASIO_HANDLER_INVOCATION_BEGIN(args) tracked_completion.invocation_begin args

#define BOOST_ASIO_HANDLER_INVOCATION_END tracked_completion.invocation_end()

#define BOOST_ASIO_HANDLER_OPERATION(args) ::custom_tracking::operation args

#define BOOST_ASIO_HANDLER_REACTOR_REGISTRATION(args) ::custom_tracking::reactor_registration args

#define BOOST_ASIO_HANDLER_REACTOR_DEREGISTRATION(args) ::custom_tracking::reactor_deregistration args
...
#define BOOST_ASIO_HANDLER_REACTOR_EVENTS(args) ::custom_tracking::reactor_events args

#define BOOST_ASIO_HANDLER_REACTOR_OPERATION(args) ::custom_tracking::reactor_operation args
...
struct custom_tracking
{
    ...
    static void creation(boost::asio::execution_context& /*ctx*/, tracked_handler& h, const char* object_type, void* /*object*/, std::uintmax_t native_handle, const char* op_name)
    {
        ...
        std::printf("Starting operation %s.%s for native_handle = %" PRIuMAX ", handler = %" PRIuMAX ", tree = %" PRIuMAX "\n", object_type, op_name, h.native_handle_, h.handler_id_, h.tree_id_);
    }

    struct completion
    {
        explicit completion(const tracked_handler& h) : handler_(h), next_(*current_completion())
        {
    ...
    {
        std::printf("Adding to reactor native_handle = %" PRIuMAX ", registration = %" PRIuMAX "\n", native_handle, registration);
    }
    ...
    {
        std::printf("Removing from reactor native_handle = %" PRIuMAX ", registration = %" PRIuMAX "\n", native_handle, registration);
    }
    ...
    {
        std::printf("Reactor readiness for registration = %" PRIuMAX ", events =%s%s%s\n", registration, (events & BOOST_ASIO_HANDLER_REACTOR_READ_EVENT) ? " read" : "",
                    (events & BOOST_ASIO_HANDLER_REACTOR_WRITE_EVENT) ? " write" : "", (events & BOOST_ASIO_HANDLER_REACTOR_ERROR_EVENT) ? " error" : "");
    }

    // Record a reactor-based operation that is associated with a handler.
    static void reactor_operation(const tracked_handler& h, const char* op_name, const boost::system::error_code& ec)
    {
        std::printf("Performed operation %s.%s for native_handle = %" PRIuMAX ", ec = %s:%d\n", h.object_type_, op_name, h.native_handle_, ec.category().name(), ec.value());
    }
    ...
    {
        std::printf("Performed operation %s.%s for native_handle = %" PRIuMAX ", ec = %s:%d, n = %" PRIuMAX "\n", h.object_type_, op_name, h.native_handle_, ec.category().name(), ec.value(),
