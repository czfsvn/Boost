...
struct async_write_message_initiation
{
    ...
    void operator()(CompletionHandler&& completion_handler, tcp::socket& socket, std::unique_ptr<std::string> encoded_message, std::size_t repeat_count,
                    std::unique_ptr<boost::asio::steady_timer> delay_timer) const
    {
        ...
        struct intermediate_completion_handler
        {
            ...
            typename std::decay<decltype(boost::asio::prefer(std::declval<tcp::socket::executor_type>(), boost::asio::execution::outstanding_work.tracked))>::type io_work_;
            ...
            void operator()(const boost::system::error_code& error, std::size_t = 0)
            {
                if (!error)
                {
                    ...
                    case waiting:
                        state_ = writing;
                        boost::asio::async_write(socket_, boost::asio::buffer(*encoded_message_), std::move(*this));
                        return;  // Composed operation not yet complete.
                    }
                }
            ...
            using executor_type = boost::asio::associated_executor_t<typename std::decay<CompletionHandler>::type, tcp::socket::executor_type>;

            executor_type get_executor() const noexcept
            {
                return boost::asio::get_associated_executor(handler_, socket_.get_executor());
            }
            ...
            // handler does not have its own.
            using allocator_type = boost::asio::associated_allocator_t<typename std::decay<CompletionHandler>::type, std::allocator<void>>;
        ...
        boost::asio::async_write(socket, encoded_message_buffer,
                                 intermediate_completion_handler{ socket, std::move(encoded_message), repeat_count, std::move(delay_timer), intermediate_completion_handler::starting,
                                                                  boost::asio::prefer(socket.get_executor(), boost::asio::execution::outstanding_work.tracked),
                                                                  std::forward<CompletionHandler>(completion_handler) });
    }
};

template <typename T, typename CompletionToken>
auto async_write_messages(tcp::socket& socket, const T& message, std::size_t repeat_count, CompletionToken&& token)
    ...
    -> typename boost::asio::async_result<typename std::decay<CompletionToken>::type, void(boost::system::error_code)>::return_type
{
    ...
    std::unique_ptr<boost::asio::steady_timer> delay_timer(new boost::asio::steady_timer(socket.get_executor()));
    ...
    return boost::asio::async_initiate<CompletionToken, void(boost::system::error_code)>(async_write_message_initiation(), token, std::ref(socket), std::move(encoded_message), repeat_count,
